                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                       <div class="grid grid-cols-1 gap-4">
                          <div class="vip_group">
                            <div class="heading">
                            <h1 class="text-center text-xl font-bold">VIP Group {{tier}}</h1>
                            <h1 class="text-center text-sm">Clicks remaining: {{$page.props.auth.user.clicks}}</h1>
                            </div>
  ...
  <div class="modal-dialog modal-dialog-centered relative w-auto pointer-events-none">
    <div class="modal-content border-none shadow-lg relative flex flex-col w-full pointer-events-auto bg-white bg-clip-padding rounded-md outline-none text-current">
      ...
      <div class="modal-body relative p-4">
        ...
        <div class="img-div">
        <img class="popup-image" :src="adToShow.img_link
">
      </div>
      <div class="product-desc">
        <p class="text-xs font-small">{{adToShow.desc}}
</p>
      </div>
      </div>
      ...
      <div class="acc-stats text-center">
        ...
        <h5>Commission: ${{(auth.user.balance + auth.user.deposit)*percentage / 30}}</h5>
        <h5>New Balance: ${{auth.user.balance + auth.user.deposit + ((auth.user.balance + auth.user.deposit)*percentage / 30)}}</h5>
      </div>
      </div>
      <div
        ...
        <button @click="sendData" type="button" data-bs-dismiss="modal"
          class="inline-block px-6 py-2.5 bg-gray-100 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out ml-1">
          Submit
        </button>
      </div>
    </div>
...
<button @click="random" type="button" class="w-64 h-64 content-center	inline-block px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded-full  shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out" data-bs-toggle="modal" data-bs-target="#clickModal">
    Start
  </button>
  </div>
                            </div>
